...
public:
    Tron1Controller() : m_running(true) {
        ...
        // 订阅模式/动作指令
        m_cmd_json_sub = nh.subscribe("/tron1/command", 10, &Tron1Controller::cmdJsonCallback, this);
...
private:
    ...
    void cmdJsonCallback(const std_msgs::String::ConstPtr& msg) {
        try {
            json cmd = json::parse(msg->data);
            sendCommand(cmd);
        } catch (const std::exception&) {
            // Fallback: treat payload as a title string.
            json cmd = {
                {"title", msg->data},
                {"data", json::object()}
            };
            ...
            if (title == "notify_imu") {
                ...
                if (msg.contains("data")) {
                    ...
                    if (data.contains("battery")) {
                        ...
                        ROS_INFO("Battery: %d%%", battery_value);
                    }
                }
            }
